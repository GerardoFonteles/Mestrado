...
Nh = 5;   % No. de neuronios na camada oculta
...
for Nh=2:30
    ...
    Media(Nh)=mean(Tx_OK)
    Mediana(Nh)=median(Tx_OK)
    [Maxima indexmax]=max(Tx_OK)
    [Minima indexmin]=min(Tx_OK)
    DevPadrao(Nh)=std(Tx_OK)
    
     %matriz de confusoes medias
    matriz_de_confusao(:,:,Nh)=calc_confusoes(Todos_diagnosticos,Todos_saidas);

    %matriz de confusoes do pior caso
    matriz_de_confusao_pior(:,:,Nh)=calc_confusoes(Todos_diagnosticos(:,indexmin),Todos_saidas(:,indexmin));

    %matriz de confusoes do melhor caso
    matriz_de_confusao_melhor(:,:,Nh)=calc_confusoes(Todos_diagnosticos(:,indexmax),Todos_saidas(:,indexmax));
end
toc
